...
public class MainMenu extends GameState {
	...
	protected void render(Graphics graphics) {
		...
		graphics.setFont(new Font("Araial", Font.PLAIN, 70));
		graphics.setColor(Color.RED);
		graphics.drawString("Maze Adventure", 20, 120);
		graphics.setFont(new Font("Araial", Font.PLAIN, 25));
		for(int i=0;i<this.optionsMenu.length;i++) {
			if(i==this.selected) graphics.setColor(Color.GREEN);
			else graphics.setColor(Color.WHITE);
			graphics.drawString(this.optionsMenu[i], 575, 245 + i * 40);
		}
		graphics.setColor(Color.RED);
		...
		graphics.setFont(new Font("Araial", Font.PLAIN, 20));
		graphics.drawString("Search the maze to find", 20, 350);
		graphics.drawString("all 12 chests.", 20, 370);
		graphics.drawString("Avoid, or burn, the bats.", 20, 390);
		graphics.drawImage(Resources.TEXTURES.get(Resources.COVER), 575, 20, Tile.SIZE*4, Tile.SIZE*4, null);
